...
def transformModel(setting,target,hip_to_eye,default_setting,ver):
    ...
    delta_lr_angle_left = (p_left + math.degrees(math.atan2(target_C, target_D_left)))/2 # 바뀐차량에서 사용자가 움직여야하는 사이드미러 좌우각도
    delta_lr_angle_right = (p_right + math.degrees(math.atan2(target_C, target_D_right)))/2 # 바뀐차량에서 사용자가 움직여야하는 사이드미러 좌우각도
    delta_ud_angle = (q - math.degrees(math.atan2(target_C, target_E)))/2 # 바뀐차량에서 사용자가 움직여야하는 사이드미러 상하각도
